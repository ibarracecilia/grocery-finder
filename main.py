...
.cart-savings{text-align:center;font-size:14px;font-weight:600;color:var(--green);margin-top:8px;padding:8px;background:var(--green-bg);border-radius:10px}
.cart-actions{display:flex;gap:10px;justify-content:center;margin-top:10px;width:100%;max-width:800px}
.cart-wa{display:flex;align-items:center;gap:8px;padding:10px 20px;border:none;border-radius:12px;background:#25D366;color:#fff;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:700;cursor:pointer;transition:all .2s;box-shadow:0 4px 12px rgba(37,211,102,.3)}
.cart-wa:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(37,211,102,.4)}
.cart-wa:active{transform:scale(.97)}
.cart-wa svg{width:20px;height:20px;fill:#fff}
...
<div class="cart-bar" id="cart-bar">
  ...
  <div class="cart-savings" id="cart-savings" style="display:none"></div>
  <div class="cart-actions" id="cart-actions" style="display:none">
    <button class="cart-wa" onclick="shareWhatsApp()">
...
function showPop(ps){document.getElementById('grid').innerHTML=ps.map((p,i)=>{const inCart=cart.includes(p.nombre);const pid=p.nombre.split(' ').join('_');return `<div class="item" style="animation-delay:${i*40}ms;animation:fadeUp .4s ease-out ${i*40}ms both"><span class="emo" onclick="pickProd('${p.nombre}')">${EM[p.nombre]||'🛒'}</span><span class="nm" onclick="pickProd('${p.nombre}')">${p.nombre}</span><span class="qt">${p.cantidad}</span><span class="pr">${t('desde')} $${p.precio_min}</span><button class="add-btn ${inCart?'added':''}" id="add-${pid}" onclick="event.stopPropagation();addToCart('${p.nombre}')">${inCart?'✓':'+'}</button></div>`}).join('')}
...
function renderCartTotals(stores){const vals=Object.values(stores).filter(v=>v>0);if(!vals.length)return;const best=Math.min(...vals);const worst=Math.max(...vals);const bestName=Object.entries(stores).find(([k,v])=>v===best)?.[0];const savings=worst-best;const sum=document.getElementById('cart-summary');sum.innerHTML=Object.entries(stores).map(([name,total])=>{const isBest=total===best&&total>0;return `<div class="cart-store ${isBest?'best-store':''}"><div class="cart-store-name">🏪 ${name}</div><div class="cart-store-total">$${total.toLocaleString('es-AR')}</div>${isBest?`<div class="cart-store-tag">${lang==='es'?'Mejor opción':'Best option'}</div>`:''}</div>`}).join('');const sEl=document.getElementById('cart-savings');const aEl=document.getElementById('cart-actions');if(savings>0){sEl.style.display='block';sEl.textContent=lang==='es'?`Ahorrás $${savings.toLocaleString('es-AR')} yendo a ${bestName}`:`Save $${savings.toLocaleString('es-AR')} shopping at ${bestName}`}else{sEl.style.display='none'}aEl.style.display='flex';document.getElementById('wa-btn-text').textContent=lang==='es'?'Compartir por WhatsApp':'Share via WhatsApp';window._cartStores=stores;window._cartBest=bestName;window._cartSavings=savings}
function shareWhatsApp(){const stores=window._cartStores;const bestName=window._cartBest;const savings=window._cartSavings;let msg=lang==='es'?'🛒 *Mi lista de GroceryFinder*\n\n':'🛒 *My GroceryFinder List*\n\n';msg+=cart.map(n=>`• ${n}`).join('\n');msg+='\n\n';msg+=lang==='es'?'💰 *Totales por supermercado:*\n':'💰 *Totals by supermarket:*\n';Object.entries(stores).forEach(([name,total])=>{const tag=name===bestName?' ✅':'';msg+=`🏪 ${name}: $${total.toLocaleString('es-AR')}${tag}\n`});if(savings>0){msg+='\n';msg+=lang==='es'?`🎉 *Ahorrás $${savings.toLocaleString('es-AR')} yendo a ${bestName}*`:`🎉 *Save $${savings.toLocaleString('es-AR')} at ${bestName}*`}msg+='\n\n🔗 grocery-finder.onrender.com';window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,'_blank')}
