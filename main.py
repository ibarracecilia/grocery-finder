  ...
  try {
    ...
    for (var k = 0; k < keys.length; k++) {
      ...
      for (var w = 0; w < ps.length; w++) {
        ...
        var arrow = wp.precio_final === best ? ' << ' + (lang === 'es' ? 'MEJOR PRECIO' : 'BEST PRICE') : '';
        waMsg += wp.supermarket + ': ' + wprice + arrow + '\n';
      }
      waMsg += '\n' + (lang === 'es' ? 'Mejor opcion: *' + bestStore + '* ($' + best + ')' : 'Best option: *' + bestStore + '* ($' + best + ')');
